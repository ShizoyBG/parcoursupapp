...
if "formation_code" not in st.session_state:
    st.session_state.formation_code = ""
if "formation_query" not in st.session_state:
    st.session_state.formation_query = ""
...
st.title("Parcoursup Data Explorer")
st.caption("Pour accéder aux données de Parcoursup sur plusieurs années, calculer des moyennes, des ratios etc...")

find_tab, explorer_tab, help_tab = st.tabs(["Trouver le code", "Explorer", "Aide"])
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Basé sur les données d'opendata fr-esr-parcoursup sur data.enseignementsup-recherche.gouv.fr.")
    ...
    if find_mode == "Code / URL":
        with st.form("code_form"):
            raw_input = st.text_input(
                ...
                placeholder="Exemple : https://www.parcoursup.fr/renseignerMesInformations?cod_aff_form=4329",
        ...
        with st.form("keyword_form"):
            ...
            keyword = st.text_input("Mot-clé", value=st.session_state.formation_query, help="Exemple : ENSEIRB, licence maths, BUT info...")
...
with explorer_tab:
    ...
    if code:
        ...
        st.info("Commencez dans l’onglet « Trouver le code » en collant une URL Parcoursup ou un code de formation.")
        st.stop()
    ...
    with c5:
        metric_card("Dernier rang appelé phase principale", latest_row.get("ran_grp1"), "Groupe 1 / phase principale")
    with c6:
        metric_card("Dernier rang appelé phase complémentaire", latest_row.get("somme_rangs"), "ran_grp1 + ran_grp2 + ran_grp3")
